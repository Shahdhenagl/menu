...
export default function DailyClosingView({ orders, expenses, language, userName, settings, allowedDrawer = null }: Props) {
  ...
  const printReport = () => {
    ...
    const section = (drawer: DrawerId) => {
      const drawerTipOrders = dayOrders.filter(order => drawerOf(order, settings) === drawer);
      const drawerExpenseRows = dayExpenses.filter(expense => (expense.drawer || 1) === drawer);
  ...
  return <div className="admin-content-section fade-in" dir={ar ? 'rtl' : 'ltr'}>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(200px,1fr))', gap: '1rem', margin: '1.2rem 0' }}>
      ...
      <Summary title={ar ? `مصروفات خزنة ${activeDrawer}` : `Drawer ${activeDrawer} expenses`} value={fmt(totalOutgoing)} />
    ...
    <div style={{ background: 'var(--bg-darker)', border: '1px solid var(--border-color)', borderRadius: 12, padding: '1.2rem' }}>
      ...
      {loading ? <p>{ar ? 'جاري التحميل…' : 'Loading…'}</p> : <><div style={{ overflowX: 'auto', marginTop: '1rem' }}><table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 680 }}><thead><tr><th>وسيلة الدفع</th><th>المحصل</th><th>الصادر</th><th>المفروض</th><th>المعدود</th><th>الفرق</th></tr></thead><tbody>{activeMethods.map(method => <tr key={method}><td>{methodLabel(method)}</td><td>{fmt(incoming[method])}</td><td>{fmt(outgoing[method])}</td><td>{fmt(expected[method])}</td><td><input className="input-gold" type="number" step="0.01" disabled={drawerClosed || method === 'partner'} value={method === 'partner' ? '' : (counted[method] || '')} onChange={e => setCounted(prev => ({ ...prev, [method]: e.target.value }))} style={{ width: 120 }} /></td><td>{fmt(method === 'partner' ? 0 : n(counted[method]) - expected[method])}</td></tr>)}<tr><td><b>{ar ? 'الإجمالي' : 'Total'}</b></td><td><b>{fmt(totalIncoming)}</b></td><td><b>{fmt(totalOutgoing)}</b></td><td><b>{fmt(totalExpected)}</b></td><td><b>{fmt(totalCounted)}</b></td><td><b>{fmt(difference)}</b></td></tr></tbody></table></div><h3 style={{ marginTop: '1.5rem', color: 'var(--gold-primary)' }}>{ar ? `حركة مصروفات خزنة ${activeDrawer}` : `Drawer ${activeDrawer} Expense Movement`}</h3><div style={{ overflowX: 'auto' }}><table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 850 }}><thead><tr><th>{ar ? 'التاريخ' : 'Date'}</th><th>{ar ? 'المصروف' : 'Expense'}</th><th>{ar ? 'النوع' : 'Type'}</th><th>{ar ? 'وسيلة الدفع' : 'Payment method'}</th><th>{ar ? 'المبلغ' : 'Amount'}</th><th>{ar ? 'الموظف' : 'Employee'}</th><th>{ar ? 'الملاحظة' : 'Notes'}</th></tr></thead><tbody>{drawerExpenses.map(expense => <tr key={expense.id}><td>{expense.expense_date || expense.created_at || '-'}</td><td>{expense.name || '-'}</td><td>{expense.type || '-'}</td><td>{methodLabel(expense.payment_method)}</td><td>{fmt(n(expense.amount))}</td><td>{expense.employee_name || expense.employee_id || '-'}</td><td>{expense.notes || '-'}</td></tr>)}{drawerExpenses.length === 0 && <tr><td colSpan={7}>{ar ? 'لا توجد مصروفات داخل هذه الخزنة' : 'No expenses in this drawer'}</td></tr>}<tr><td colSpan={4}><b>{ar ? 'إجمالي مصروفات الخزنة' : 'Drawer expense total'}</b></td><td><b>{fmt(totalOutgoing)}</b></td><td colSpan={2}></td></tr></tbody></table></div><textarea className="input-gold" disabled={drawerClosed} value={notes} onChange={e => setNotes(e.target.value)} placeholder={ar ? 'ملاحظات التقفيل…' : 'Closing notes…'} style={{ width: '100%', marginTop: '1rem', minHeight: 70 }} /><div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '1rem' }}><button className="btn-gold" disabled={saving || drawerClosed || Boolean(openTableOrders.length)} onClick={closeDrawer}>{drawerClosed ? <><Lock size={16} /> {ar ? 'الخزنة مقفولة' : 'Drawer closed'}</> : <>{ar ? `إغلاق خزنة ${activeDrawer}` : `Close Drawer ${activeDrawer}`}</>}</button></div></>}
    </div>
  </div>;
}
